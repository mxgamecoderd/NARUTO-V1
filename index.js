...
async function sessionLoader() {
  try {
    ...
    if (!fs.existsSync(credsPath)) {
      ...
      const sessionData = global.SESSION_ID.split("NARUTO-V1~")[1];
...
✉️  Script : NARUTO-V1
✉️  Author : MX-GΔMΞCØDΞR
✉️  Gmail : olamilekandamilaraa@gmail.com
✉️  Instagram : mxgamecoder

${chalk.green.bold("Ｐｏｗｅｒｅｄ Ｂｙ ＮＡＲＵＴＯ")}\n`));
...
async function startXliconBot() {
    ...
    if (pairingCode && !XliconBotInc.authState.creds.registered) {
        ...
        let phoneNumber;
        phoneNumber = await question('Please enter your number starting with country code like 23:\n');
